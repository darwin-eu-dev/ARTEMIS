...
#' @param patientDrugRecord A complete patient drug record
#' @export
#' @return A data.table object with drugs
#'
createDrugDF <- function(patientDrugRecord) {

  drugRec <- encode(patientDrugRecord)
...
#' 
calculateRegimenTimes <- function(df, drugDF) {
...
#' @param df A dataframe created by align() with cumulative times added by calculateRegimenTimes()
#' @return A data.table object without removed overlaps
...
combineOverlaps <- function(df, regimenCombine) {
    ...
    dt <- dt[, .(
              ...
              alignmentID   = paste(alignmentID, collapse = ";")
...
postprocessSinglePatientDF <- function(output, regimenCombine = 28, discontinuationTime = regimenCombine) {
    ...
    # Use patient drug record to create regimen times
    # Assume drug record is the same for all rows
    drugDF <- createDrugDF(patientDrugRecord = df$CompleteDrugRecord[1])

    df <- calculateRegimenTimes(df, drugDF)    
